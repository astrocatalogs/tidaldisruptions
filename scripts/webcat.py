...
from astrocats.tidaldisruptions.scripts.events import (get_event_filename,
                                                       get_event_text)
from astrocats.tidaldisruptions.scripts.repos import (get_rep_folder,
                                                      repo_file_list)
from cdecimal import Decimal

parser = argparse.ArgumentParser(
    description='Generate a catalog JSON file and plot HTML files from TDE data.')
parser.add_argument(
    '--no-write-catalog',
    '-nwc',
    dest='writecatalog',
...
googlepingurl = "http://www.google.com/webmasters/tools/ping?sitemap=https%3A%2F%2Ftde.space%2Fsitemap.xml"
...
titles = [
    ...
    "Host Name", "TDE J2000 Right Ascension (h:m:s)",
    "TDE J2000 Declination (d:m:s)", "Host J2000 Right Ascension (h:m:s)",
...
with open('astrocats/tidaldisruptions/html/sitemap-template.xml', 'r') as f:
...
tdepages = [["# name", "aliases", "max apparent mag", "max mag date",
...
for fcnt, eventfile in enumerate(tq(sorted(files, key=lambda s: s.lower()))):
        ...
        try:
            ...
            if eventname in hostimgdict:
                ...
                if hasimage and filecmp.cmp(
                        ...
                        'astrocats/tidaldisruptions/input/missing.jpg'):
                    hasimage = False
    ...
    if dohtml and args.writehtml:
        ...
        html = re.sub(r'(\<\/title\>)', r'''\1\n
            <base target="_parent" />\n
            <link rel="stylesheet" href="https://tde.space/wp-content/themes/astrocats-child-theme/event-iframe.css" type="text/css">\n
            <script type="text/javascript" src="https://tde.space/astrocats/astrocats/tidaldisruptions/scripts/marks.js" type="text/js"></script>\n
        ...
        html = re.sub(
            ...
            r'<a href="https://github.com/astrocatalogs/tidaldisruptions/issues/new'
                    ...
                    for r, row in enumerate(catalog[entry][key]):
                        if 'value' in row and 'source' in row:
                            ...
                            edit = "true" if os.path.isfile(
                                'astrocats/tidaldisruptions/input/tde-internal/'
                                + get_event_filename(
    ...
    if args.writecatalog:
        ...
        tdepages.append(
            [entry, ",".join([x['value'] for x in catalog[entry]['alias']]),
             ...
             catalog[entry]['numphoto'], 'https://tde.space/' + plotlink])
...
if args.writecatalog and not args.eventlist:
    ...
    if not args.boneyard:
        ...
        with open(outdir + htmldir + 'tdepages.csv' + testsuffix, 'w') as f:
        ...
        with open(
                outdir + htmldir + 'info-snippets/tdecount.html' + testsuffix,
                'w') as f:
