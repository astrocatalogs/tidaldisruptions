...
def do_wiserep_spectra(catalog):
    if not catalog.args.travis:
        ...
        spider(update=True, daysago=7, path="/../../tde-external-WISEREP/")
    ...
    for folder in pbar_strings(file_names, task_str):
        ...
        catalog.entries[name].add_quantity(
            TIDALDISRUPTION.ALIAS, name, secondarysource)
        ...
        for fname in pbar(files, task_str):
            ...
            if claimedtype not in ['Other']:
                catalog.entries[name].add_quantity(
                    TIDALDISRUPTION.CLAIMED_TYPE, claimedtype,
                    secondarysource)
            catalog.entries[name].add_quantity(
                TIDALDISRUPTION.REDSHIFT, redshift, secondarysource)
