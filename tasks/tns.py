    ...
    for page in pbar(range(maxpages), task_str):
        ...
        for ri, row in enumerate(pbar(tsvin, task_str, leave=False)):
            ...
            catalog.entries[name].add_quantity(
                TIDALDISRUPTION.ALIAS, name, source)
            if row[2] and row[2] != '00:00:00.00':
                catalog.entries[name].add_quantity(TIDALDISRUPTION.RA, row[2],
                                                   source)
            if row[3] and row[3] != '+00:00:00.00':
                catalog.entries[name].add_quantity(TIDALDISRUPTION.DEC, row[3],
                                                   source)
            if row[4]:
                catalog.entries[name].add_quantity(
                    TIDALDISRUPTION.CLAIMED_TYPE, row[4].replace(
                        'SN', '').strip(),
                    source)
            if row[5]:
                catalog.entries[name].add_quantity(
                    TIDALDISRUPTION.REDSHIFT, row[5], source,
                    kind='spectroscopic')
            if row[6]:
                catalog.entries[name].add_quantity(
                    TIDALDISRUPTION.HOST, row[6], source)
            if row[7]:
                catalog.entries[name].add_quantity(
                    ...
                    kind='host')
            if row[8]:
                catalog.entries[name].add_quantity(TIDALDISRUPTION.DISCOVERER,
                                                   row[8], source)
